...
void LCD_Init() {
    ...
    SysTick_Init(30000); //10ms         // Volume 1 Program 4.7, Volume 2 Program 2.12
...
// Output: none
    ...
    void LCD_OutString(char *pt) {
         uint8_t pt_length = strlen(pt); //Iteratively prints out the string one char at a time
...
//resource : https://www.tutorialspoint.com/c_standard_library/c_function_sprintf.htm

     char dec[12];
     //array dec %d number
     sprintf(dec, "%d", n); //converts number to unsigned hexi-decimal
     LCD_OutString(dec);
...
//resource : https://www.tutorialspoint.com/c_standard_library/c_function_sprintf.htm
    char hex[12];
    sprintf(hex, "%X", number);//converts number to unsigned hexi-decimal
    LCD_OutString(hex);
}
...
void LCD_OutUFix(uint32_t number) {
    ...
    if (number < 0 || number >9999) //if out of range
    {
        LCD_OutString("*.***");
    }
     else
         {
         char fix[12];
         double num = number/1000.0; //double to store 32,converts to decimal
         sprintf(fix,"%.3lf", num);//3 decimal places rounded long integer fixed point number
         LCD_OutString(fix);//output fixed point number
         }
     }
    }//end of LCD_OutFix
